...
for i in range(8):
    ...
    img_target_a.append(cv2.Canny(temp, 100, 200,
                                  apertureSize=3, L2gradient=True))

for i in range(8):
    temp = cv2.imread("target_rsz/class_B_" + str(i*45) + ".jpg")
    img_target_b.append(cv2.Canny(temp, 100, 200,
                                  apertureSize=3, L2gradient=True))
    
for i in range(8):
    temp = cv2.imread("target_rsz/class_C_" + str(i*45) + ".jpg")
    img_target_c.append(cv2.Canny(temp, 100, 200,
                                  apertureSize=3, L2gradient=True))

for i in range(8):
    temp = cv2.imread("target_rsz/class_D_" + str(i*45) + ".jpg")
    img_target_d.append(cv2.Canny(temp, 100, 200,
                                  apertureSize=3, L2gradient=True))
...
while True:
    ...
    if ret == True:       
        ...
        frame = cv2.Canny(frame, 80, 200, apertureSize=3, L2gradient=True)
