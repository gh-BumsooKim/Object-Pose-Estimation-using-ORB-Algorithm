...
for i in range(8):
    ...
    temp1 = cv2.Sobel(temp, cv2.CV_8U, 1, 0, 3)
    temp2 = cv2.Sobel(temp, cv2.CV_8U, 0, 1, 3)
    img_target_a.append(temp1 + temp2)
...
for i in range(8):
    ...
    temp1 = cv2.Sobel(temp, cv2.CV_8U, 1, 0, 3)
    temp2 = cv2.Sobel(temp, cv2.CV_8U, 0, 1, 3)
    img_target_b.append(temp1 + temp2)
...
for i in range(8):
    ...
    temp1 = cv2.Sobel(temp, cv2.CV_8U, 1, 0, 3)
    temp2 = cv2.Sobel(temp, cv2.CV_8U, 0, 1, 3)
    img_target_c.append(temp1 + temp2)
...
for i in range(8):
    ...
    temp1 = cv2.Sobel(temp, cv2.CV_8U, 1, 0, 3)
    temp2 = cv2.Sobel(temp, cv2.CV_8U, 0, 1, 3)
    img_target_d.append(temp1 + temp2)
...
while True:
    ...
    if ret == True:
        frame1 = cv2.Sobel(cv2.cvtColor(frame,cv2.COLOR_BGR2GRAY),
                          -1, 0, 1, 3)
        
        frame2 = cv2.Sobel(cv2.cvtColor(frame,cv2.COLOR_BGR2GRAY),
                          -1, 1, 0, 3)        
        frame = frame1 + frame2
