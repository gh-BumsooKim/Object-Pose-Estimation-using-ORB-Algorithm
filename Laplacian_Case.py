...
for i in range(8):
    ...
    img_target_a.append(cv2.Laplacian(cv2.cvtColor(temp,cv2.COLOR_BGR2GRAY),
                         -1, 5))

for i in range(8):
    temp = cv2.imread("target_rsz/class_B_" + str(i*45) + ".jpg")
    img_target_b.append(cv2.Laplacian(cv2.cvtColor(temp,cv2.COLOR_BGR2GRAY),
                         -1, 5))
    
for i in range(8):  
    temp = cv2.imread("target_rsz/class_C_" + str(i*45) + ".jpg")
    img_target_c.append(cv2.Laplacian(cv2.cvtColor(temp,cv2.COLOR_BGR2GRAY),
                         -1, 5))

for i in range(8):
    temp = cv2.imread("target_rsz/class_D_" + str(i*45) + ".jpg")
    img_target_d.append(cv2.Laplacian(cv2.cvtColor(temp,cv2.COLOR_BGR2GRAY),
                         -1, 5))
